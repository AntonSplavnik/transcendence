...
export class SwingTrail {
	...
	private prewarmShader(): void {
		...
		const warmup = MeshBuilder.CreateRibbon(
			'_swingTrailWarm',
			{
				pathArray: [p, p],
			},
	...
	private rebuildRibbon(): void {
		...
		for (let i = 0; i < n; i++) {
			const age = i / (n - 1);
			const r =
				this.config.baseColor.r + (this.config.tipColor.r - this.config.baseColor.r) * age;
			const g =
				this.config.baseColor.g + (this.config.tipColor.g - this.config.baseColor.g) * age;
			const b =
				this.config.baseColor.b + (this.config.tipColor.b - this.config.baseColor.b) * age;
			const a =
				this.config.tailOpacity + (this.config.tipOpacity - this.config.tailOpacity) * age;
