...
On each `checkAndReload()` call, iterate `GameConfig::PRESETS_DIR`
(`"assets/presets"`) using `std::filesystem::recursive_directory_iterator`
(matching how `CharacterPresetRegistry::loadFromDirectory` scans at startup,
...
mtime is newer or that are not yet in the map (new files).
...
The call must be wrapped in `try { ... } catch (const std::exception& e)`:
`loadFromFile` throws `std::runtime_error` on file-open failure, JSON parse
error, schema validation failure, or id mismatch
(`CharacterPresetLoader.hpp:243-276`). An uncaught exception would propagate
through the CXX bridge and panic the game loop thread.
...
For each reloaded preset id, iterate `registry.view<PresetBinding>()` and
filter to entities whose `PresetBinding::id` matches. Replace their
preset-sourced components:
...
~80 lines. Single class, header-only (matching codebase convention).
...
Constructor stores references. `checkAndReload()` performs steps 1-4.
The entire file is wrapped in `#ifndef NDEBUG` / `#endif`.
...
| Exception handling | Implicit ("log the error and skip") | Explicit `try/catch (std::exception&)` required |
