#pragma once

#include "Systems/SystemEnTT.hpp"
#include <vector>
#include <memory>
#include <algorithm>
#include <cstring>

namespace ArenaGame {

// =============================================================================
// SystemManagerEnTT - Manages EnTT-based game systems
// =============================================================================
// Drop-in replacement for SystemManager that works with SystemEnTT instances
// - Identical interface to SystemManager
// - Manages SystemEnTT lifecycle
// - Handles multi-phase updates
//
// Update phases (in order):
//   1. start() - Called once after initialization
//   2. earlyUpdate() - Before physics (input)
//   3. fixedUpdate() - Physics (fixed timestep, may run 0-N times per frame)
//   4. update() - Game logic (every frame)
//   5. lateUpdate() - Post-processing (every frame)
//
// Usage:
//   SystemManagerEnTT manager;
//   manager.addSystem(std::make_unique<PhysicsSystemEnTT>());
//   manager.addSystem(std::make_unique<CollisionSystemEnTT>());
//   manager.initialize();
//   manager.start();
//
//   // Game loop
//   while (running) {
//       manager.earlyUpdate(deltaTime);
//       while (shouldDoFixedUpdate()) {
//           manager.fixedUpdate(fixedDeltaTime);
//       }
//       manager.update(deltaTime);
//       manager.lateUpdate(deltaTime);
//   }
// =============================================================================

class SystemManagerEnTT {
public:
    SystemManagerEnTT() = default;
    ~SystemManagerEnTT() = default;

    // System management
    void addSystem(std::unique_ptr<SystemEnTT> system);
    void removeSystem(const char* systemName);
    SystemEnTT* getSystem(const char* systemName);

    // Get system by type (template convenience method)
    template<typename T>
    T* getSystem() {
        for (auto& system : m_systems) {
            T* casted = dynamic_cast<T*>(system.get());
            if (casted) {
                return casted;
            }
        }
        return nullptr;
    }

    // Lifecycle
    void initialize();
    void start();
    void shutdown();
    void clear();

    // Update phases
    void earlyUpdate(float deltaTime);
    void fixedUpdate(float fixedDeltaTime);
    void update(float deltaTime);
    void lateUpdate(float deltaTime);

    // Queries
    size_t getSystemCount() const { return m_systems.size(); }
    bool isInitialized() const { return m_initialized; }
    bool isStarted() const { return m_started; }

private:
    std::vector<std::unique_ptr<SystemEnTT>> m_systems;
    bool m_initialized = false;
    bool m_started = false;
};

// =============================================================================
// Implementation
// =============================================================================

inline void SystemManagerEnTT::addSystem(std::unique_ptr<SystemEnTT> system) {
    if (!system) {
        return;
    }

    // Initialize system if manager is already initialized
    if (m_initialized) {
        system->initialize();
        if (m_started) {
            system->start();
        }
    }

    m_systems.push_back(std::move(system));
}

inline void SystemManagerEnTT::removeSystem(const char* systemName) {
    m_systems.erase(
        std::remove_if(m_systems.begin(), m_systems.end(),
            [systemName](const std::unique_ptr<SystemEnTT>& system) {
                bool match = std::strcmp(system->getName(), systemName) == 0;
                if (match) {
                    system->shutdown();
                }
                return match;
            }
        ),
        m_systems.end()
    );
}

inline SystemEnTT* SystemManagerEnTT::getSystem(const char* systemName) {
    for (auto& system : m_systems) {
        if (std::strcmp(system->getName(), systemName) == 0) {
            return system.get();
        }
    }
    return nullptr;
}

inline void SystemManagerEnTT::initialize() {
    if (m_initialized) {
        return;
    }

    for (auto& system : m_systems) {
        system->initialize();
    }

    m_initialized = true;
}

inline void SystemManagerEnTT::start() {
    if (!m_initialized || m_started) {
        return;
    }

    for (auto& system : m_systems) {
        system->start();
    }

    m_started = true;
}

inline void SystemManagerEnTT::shutdown() {
    if (!m_initialized) {
        return;
    }

    // Shutdown in reverse order
    for (auto it = m_systems.rbegin(); it != m_systems.rend(); ++it) {
        (*it)->shutdown();
    }

    m_initialized = false;
    m_started = false;
}

inline void SystemManagerEnTT::clear() {
    shutdown();
    m_systems.clear();
}

inline void SystemManagerEnTT::earlyUpdate(float deltaTime) {
    for (auto& system : m_systems) {
        if (system->needsEarlyUpdate()) {
            system->earlyUpdate(deltaTime);
        }
    }
}

inline void SystemManagerEnTT::fixedUpdate(float fixedDeltaTime) {
    for (auto& system : m_systems) {
        if (system->needsFixedUpdate()) {
            system->fixedUpdate(fixedDeltaTime);
        }
    }
}

inline void SystemManagerEnTT::update(float deltaTime) {
    for (auto& system : m_systems) {
        if (system->needsUpdate()) {
            system->update(deltaTime);
        }
    }
}

inline void SystemManagerEnTT::lateUpdate(float deltaTime) {
    for (auto& system : m_systems) {
        if (system->needsLateUpdate()) {
            system->lateUpdate(deltaTime);
        }
    }
}

} // namespace ArenaGame
